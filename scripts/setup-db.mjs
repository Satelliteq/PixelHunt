...
async function setupDatabase() {
  ...
  try {
    ...
    try {
      // Eğer tabloda data varsa, tablo zaten dolu demektir
      const existingCategories = await client.query(`
        SELECT * FROM categories LIMIT 5;
      `);
      
      if (existingCategories.rows.length > 0) {
        console.log('Kategoriler zaten mevcut:', existingCategories.rows.length);
      } else {
        // Tablo boş, kategorileri ekleyelim
        console.log('Kategoriler ekleniyor...');
        
        // Her kategoriyi ayrı ayrı eklemeyi deneyelim
        for (const category of [
          ...
          try {
            ...
            // Duplikasyon hatası muhtemelen kategori zaten var
            if (err.code === '23505') {
              console.log(`Kategori zaten mevcut: ${category.name}`);
            } else {
              console.error(`Kategori eklenirken hata: ${category.name}`, err);
            }
          }
        }
      }
    } catch (categoryError) {
      console.error('Kategori kontrolü sırasında hata:', categoryError);
    }
    
    console.log('Kategori işlemleri tamamlandı.');
